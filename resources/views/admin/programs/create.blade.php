...
<div class="max-w-2xl mx-auto">
    <h1 class="text-2xl font-semibold mb-6">Create Program</h1>

    <form method="POST" action="{{ route('admin.programs.store') }}" class="space-y-4 bg-white p-6 rounded-lg shadow">
        @csrf
        ...
        <div>
            ...
            <textarea id="majors" name="majors" rows="3" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="List majors separated by commas">{{ old('majors') }}</textarea>
            ...
            <div class="mt-2 max-h-60 overflow-y-auto border border-gray-300 rounded-md p-4">
                @forelse($courses as $course)
                    <label class="flex items-center py-2">
                        <input type="checkbox" name="courses[]" value="{{ $course->id }}" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
        ...
        <div class="flex items-center justify-end gap-4">
            ...
            <x-primary-button>Create Program</x-primary-button>
        </div>
    </form>
</div>
@endsection
