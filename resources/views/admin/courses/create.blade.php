...
<div class="max-w-2xl mx-auto">
    <h1 class="text-2xl font-semibold mb-6">Create Course</h1>

    <form method="POST" action="{{ route('admin.courses.store') }}" class="space-y-4 bg-white p-6 rounded-lg shadow">
        @csrf
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <x-text-input id="lec_unit" class="block mt-1 w-full" type="number" name="lec_unit" :value="old('lec_unit', 0)" required min="0" />
                <x-input-error :messages="$errors->get('lec_unit')" class="mt-2" />
            </div>

            <div>
                <x-input-label for="lab_unit" :value="__('Lab Unit')" />
                <x-text-input id="lab_unit" class="block mt-1 w-full" type="number" name="lab_unit" :value="old('lab_unit', 0)" required min="0" />
